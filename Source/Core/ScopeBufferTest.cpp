...
public:
    ...
    void runTest() override
    {
        ...
        // Push 100 samples with 4 beats per pass at 120 bpm, starting at 0 ppq
        buffer.pushBlock(testBuffer, 1, 0.0, 120.0, 4.0);

        // Verification: The peak should be captured in a range
        bool foundPeak = false;
        for (int i = 0; i < ScopeBuffer::NumBins; ++i)
        {
            auto range = buffer.getBinRange(i);
            if (range.getEnd() >= 1.0f)
            {
                foundPeak = true;
                break;
            }
        ...
        buffer.reset();
        expectEquals(buffer.getWriteIndex(), 0);
        expectEquals(buffer.getBinRange(0).getEnd(), 0.0f);
    }
};
